...
$sqlcek = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_paketsoal where XKodeSoal = '$_REQUEST[txt_namax]'");
...
if($jumcek<1){
			...
			$sqlsoal = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$_REQUEST[txt_ujianx]'");
			$jumsql = mysqli_num_rows($sqlsoal);
			if($jumsql>0){
				while($r = mysqli_fetch_array($sqlsoal)){
				$str_tanya = str_replace("'","\'",$r['XTanya']);
					 $query = mysqli_query($GLOBALS["___mysqli_ston"], "insert into cbt_soal (XNomerSoal, XKodeMapel, XKodeSoal, XTanya, XJawab1, XGambarJawab1, XJawab2,XGambarJawab2, 
