...
$query0 = "TRUNCATE TABLE cbt_siswa";
		  $hasil0 = mysqli_query($GLOBALS["___mysqli_ston"], $query0);
...
for ($i=3; $i<=$baris; $i++)
{
 ...
 if(!str_replace(" ","",$xnomer)==""){ 
		  ...
		  $hasilkelas = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], $querykelas));
		  $queryjur = "select XKodeJurusan from cbt_kelas where  XKodeJurusan = '$xjur'";
		  $hasiljur = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], $queryjur));
		  $querylevel = "select XKodeLevel from cbt_kelas where XKodeLevel = '$xlevel' ";
		  $hasillevel = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], $querylevel));
		  $querynamakelas = "select XNamaKelas from cbt_kelas where XNamaKelas = '$xnakel' ";
		  $hasilnamakelas = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], $querynamakelas));
		  $querykode = mysqli_query($GLOBALS["___mysqli_ston"], "select XKodeSekolah from cbt_admin");
