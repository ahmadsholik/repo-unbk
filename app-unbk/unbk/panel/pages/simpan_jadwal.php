...
// Ambil Paket Soal
//=========================
$loop = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_paketsoal where XStatusSoal ='Y' and XKodeSoal = '$_REQUEST[txt_kodesoal]'");
while($s = mysqli_fetch_array($loop)){
$val_jumsoal = $s['XJumSoal'];
$val_pilganda = $s['XPilGanda'];
$val_esai = $s['XEsai'];

	$sqlubah = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_ujian where XKodeSoal = '$_REQUEST[txt_kodesoal]' and  XKodeUjian = '$_REQUEST[txt_ujian]' and 
...
$jumsoal = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where  XKodeSoal = '$_REQUEST[txt_kodesoal]'"));
...
//
//=================================
$sqls = mysqli_query($GLOBALS["___mysqli_ston"], "select u.*,m.*,u.Urut as Urutan,u.XKodeKelas as kokel from cbt_ujian u left join cbt_mapel m on m.XKodeMapel = u.XKodeMapel 
...
$sqlcek = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_ujian where XTokenUjian = '$_REQUEST[txt_token]'"));
	if($sqlcek>0){echo "<div class='alert alert-danger alert-dismissable' id='ndelik'>Simpan Data Gagal Token Sudah ada.</div>     ";
	} else {
				$sqlinsert = mysqli_query($GLOBALS["___mysqli_ston"], "insert into cbt_ujian 						  
