...
<?php include "../../config/server.php"; 
					...
					$sqlsoal = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$_REQUEST[soal]'"));
					$sqlpakai = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_siswa_ujian where XKodeSoal = '$_REQUEST[soal]' and XStatusUjian = '1'"));
					$sqlsudah = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_jawaban where XKodeSoal = '$_REQUEST[soal]'"));


if(isset($_REQUEST['aksi'])&&$_REQUEST['aksi']=="hapus"){$sqldel = mysqli_query($GLOBALS["___mysqli_ston"], "delete from cbt_soal where Urut = '$_REQUEST[nomer]' and XKodeSoal = '$_REQUEST[soal]'");}
?>
            <div class="portlet light ">
                                <div class="portlet-title">
                                    <div class="caption">
                                        Edit Daftar Bank Soal
                                    </div>
									<div class="actions">
    ...
    <?php } ?>
	<a type="button" class="btn btn-dark circle btn-icon-only btn-default fullscreen" href="javascript:;" data-original-title="" title=""> </a>
                        ...
                        <div class="portlet-body">
                             <table id="example" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
<?php
$sql0 = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal= '$_REQUEST[soal]' order by XnomerSoal");
...
$sqlsoal = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$xadm[XKodeSoal]'"));
$str = $xadm['XTanya'];
...
<button type='button' class='btn btn-default'  data-toggle='modal' data-target='#myModalR$xadm[Urut]' alt='Lihat'><i class='fa fa-search'></i> </button>
					...
					$sqlsudah1 = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_jawaban where XKodeSoal = '$xadm[XKodeSoal]' and XNomerSoal = '$xadm[XNomerSoal]'"));
                        ...
                        <div class="row" style="background-color:#fff">
                            <div class="col-xs-12">
                                <div class="wysiwyg-content" style="width:100%">
									...
									$sqlprev = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$xadm[XKodeSoal]' and Urut = '$xadm[Urut]'");
