...
$sqlujian = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from cbt_jawaban WHERE XKodeSoal = '$_REQUEST[soal]'");
...
$hasil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cbt_siswa "); }
elseif($txt_kelas == 'ALL' && $txt_jurusan !== 'ALL'){
$hasil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cbt_siswa where XKodeJurusan = '$txt_jurusan'"); }
elseif( $txt_kelas !== 'ALL' && $txt_jurusan == 'ALL'){
$hasil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cbt_siswa where XKodeKelas = '$txt_kelas'"); 
} else {
$hasil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cbt_siswa where XKodeKelas = '$txt_kelas'");
}
...
while($p = mysqli_fetch_array($hasil)){
	...
	$sqlpaket = mysqli_query($GLOBALS["___mysqli_ston"], "select * from  cbt_paketsoal where XKodeSoal = '$var_soal'"); 	
...
$sqlmapel = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_ujian c left join cbt_mapel m on m.XKodeMapel = c.XKodeMapel where c.XKodeSoal = '$var_soal' and c.XKodeMapel = '$var_mapel'"); 
	$sqlmapel = mysqli_query($GLOBALS["___mysqli_ston"], "select * from  cbt_mapel where XKodeMapel = '$var_mapel'"); 	
	$u = mysqli_fetch_array($sqlmapel);
	$namamapel = $u['XNamaMapel'];	
	
	$sqldijawab = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `cbt_jawaban` WHERE XKodeSoal = '$var_soal' and XUserJawab = '$var_siswa' and XJawaban != '' and XTokenUjian = 
	'$var_token'"));

$sqljawaban = mysqli_query($GLOBALS["___mysqli_ston"], " SELECT count( XNilai ) AS HasilUjian FROM `cbt_jawaban` WHERE XKodeSoal = '$var_soal' and XUserJawab = '$var_siswa' and XNilai = '1' and XTokenUjian = '$var_token'");
