...
$sqlujian = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from cbt_ujian WHERE XKodeSoal = '$_REQUEST[soal]'");
...
while($p = mysqli_fetch_array($hasil)){
	...
	$sqlpaket = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_paketsoal p LEFT JOIN cbt_mapel m on m.XKodeMapel=p.XKodeMapel where p.XKodeSoal = '$var_soal'"); 	
...
$sqldijawab = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `cbt_jawaban` WHERE XKodeSoal = '$var_soal' and XUserJawab = '$var_siswa' and XJawaban != ''"));
	$sqljawaban = mysqli_query($GLOBALS["___mysqli_ston"], " SELECT count( XNilai ) AS HasilUjian,XTokenUjian FROM `cbt_jawaban` WHERE XKodeSoal = '$var_soal' and XUserJawab = '$var_siswa' and XNilai = 
	'1' ");
