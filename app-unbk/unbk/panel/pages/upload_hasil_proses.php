...
$sql4 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from cbt_ujian WHERE XTokenUjian='$token'");
...
if(isset($_REQUEST['upl'])){ 
	$sqlnilai = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_nilai where XTokenUjian = '$_REQUEST[token]' AND XStatus='0' or XStatus='1'");
	
	while($nilai=mysqli_fetch_array($sqlnilai)){
		include "../../config/server_pusat.php";
		$cek = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_nilai where  XTokenUjian = '$_REQUEST[token]' and XNomerUjian='$nilai[XNomerUjian]'"));
		if($cek==0){
		$sql = mysqli_query($GLOBALS["___mysqli_ston"], "insert into cbt_nilai (XNomerUjian,XNIK,XKodeUjian,XTokenUjian,XTgl,XJumSoal,XBenar,XSalah,XNilai,XPersenPil,XPersenEsai,
	...
	include "../../config/server.php";
	$sqljawab = mysqli_query($GLOBALS["___mysqli_ston"], "update cbt_nilai SET XStatus='9' where XNomerUjian = '$nilai[XNomerUjian]' and  XTokenUjian = '$_REQUEST[token]'");
...
<button type="button" class="btn btn-success"><i class='fa fa-refresh fa-fw'></i></button></a>
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover">
                                ...
                                <?php   
								include "../../config/server.php";
								$sql5 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from cbt_nilai
