...
$sqltoken = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM cbt_ujian where XStatusUjian = '1'");
$t = mysqli_fetch_array($sqltoken);
$tokenujian = $t['XTokenUjian'];


$sql = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT *,u.XStatusUjian as ujsta,c.XTokenUjian as tokenz,u.XNomerUjian as noujian
FROM cbt_siswa s
...
elseif($staujian =='9'){$staujian = "Tes SELESAI";}
	$sqldijawab = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], " SELECT * FROM `cbt_jawaban` WHERE XTokenUjian = '$tokenujian' and XJawaban != '' and XUserJawab = '$nouji'"));
	$sqljawaban = mysqli_query($GLOBALS["___mysqli_ston"], " SELECT count( XNilai ) AS HasilUjian FROM `cbt_jawaban` WHERE XNilai = '1' and XTokenUjian = '$tokenujian' and XUserJawab = '$nouji'");
