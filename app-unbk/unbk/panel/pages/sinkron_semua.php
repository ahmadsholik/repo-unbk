...
//document.getElementById("information").innerHTML="  Sikronisasi : DATA 1 ... <b>'.$i.'</b> dari <b>'. $baris.'</b> Selesai.";
		...
		$sqlcek = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_paketsoal order by Urut");
...
$i = 1;
		...
		include "../../config/server_pusat.php";
		$sqlcek = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_mapel order by Urut");
		$baris = mysqli_num_rows($sqlcek);
		//echo "jumlah total paket data : $baris";
		...
		//for ($i=1; $i<=$baris; $i++){
					...
					$query =  mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO cbt_mapel ( XKodeMapel, XNamaMapel,XPersenUH,XPersenUTS,XPersenUAS,XKKM) VALUES ('$r[XKodeMapel]', '$r[XNamaMapel]', '$r[XPersenUH]', '$r[XPersenUTS]', '$r[XPersenUAS]', '$r[XKKM]')");
...
$i = 1;
		...
		include "../../config/server_pusat.php";
		$sqlcek = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_siswa order by Urut");
		$baris = mysqli_num_rows($sqlcek);
		//echo "jumlah total paket data : $baris";
		...
		//for ($i=1; $i<=$baris; $i++){
		  ...
		  '$r[XKodeLevel]','$r[XFoto]','$r[XAgama]','$r[XSetId]')");*/
					$query =  mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO cbt_siswa (XNomerUjian, XNIK,XSesi,XRuang, XNamaSiswa,XKodeKelas, XJenisKelamin, XPassword, XKodeJurusan,
