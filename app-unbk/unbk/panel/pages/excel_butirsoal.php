...
<?php 
$soal = $_REQUEST['soal'];
$sql0 = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$soal' and XJenisSoal = '1' order by XNomerSoal"));
$jumkol = $sql0+7;
$sqlsoalan = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal s left join cbt_mapel m on s.XKodeMapel = m.XKodeMapel 
...
<?php 
$sql = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal = '$soal' and XJenisSoal = '1' order by XNomerSoal");
$no =1;
...
*/
//jawaban siswa
$sqljwb = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_jawaban j left join cbt_siswa s on s.XNomerUjian = j.XUserJawab where XKodeSoal = '$soal' 
and j.XJenisSoal = '1' group by XUserJawab order by XUserJawab");
$nom =1;
					$sqlujian = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_ujian where XKodeSoal = '$soal' ");
...
<td style='border-right:thin solid #000;  border-bottom:thin solid #000 '>&nbsp; $sj[XNamaSiswa]</td>";
		$sql0 = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_soal where XKodeSoal ='$soal'  and XJenisSoal = '1' order by XNomerSoal");
	...
	$sql1 = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_jawaban where XUserJawab = '$sj[XUserJawab]' and XNomerSoal = '$s0[XNomerSoal]' and XKodeSoal = '$soal' ");
					...
					$sql2 = mysqli_query($GLOBALS["___mysqli_ston"], "select sum(XNilai) as skor2 from cbt_jawaban where XUserJawab = '$sj[XUserJawab]' and XKodeSoal = '$soal' and XJenisSoal = '1'");
					...
					$sqlkkm = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_mapel where XKodeMapel = '$mapel'");
