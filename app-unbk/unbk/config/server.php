...
$sqlconn=@($GLOBALS["___mysqli_ston"] = mysqli_connect($db_host, $db_user, $db_pass));

// 2. Pilih database
$db_selected=mysqli_select_db( $sqlconn, $db);

// 3. Periksa database jika telah siap
$val = mysqli_query($GLOBALS["___mysqli_ston"], 'Select * from cbt_admin LIMIT 1');
if($val == TRUE){
	...
	$xadmz = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_zona LIMIT 1"));
	$zo= $xadmz['XZona'];
	}
	else {	$zo="Asia/Jakarta";}
	date_default_timezone_set($zo);
	$xadm = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_server LIMIT 1"));
